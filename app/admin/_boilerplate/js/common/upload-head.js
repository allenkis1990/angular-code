...
define(['webuploader', 'cropper'],
    function (WebUploader) {
        ...
        return ['$timeout', '$log', '$rootScope', 'HB_dialog',
            function ($timeout, $log, $rootScope, HB_dialog) {
                var options = $rootScope.uploadConfigOptions;
                var baseFileSize=524288;
                var baseTip=500;
                ...
                return {
                    ...
                    templateUrl: function ($element, $attr) {
                        return $attr.templateUrl || '@systemUrl@/templates/tpls/tpl-upload-head-img-tpl.html';
                    },
                    link: function ($scope, element, attrs, ngModuleCtrl) {
                        ...
                        function uploadImg () {
                            var Info = imagePreviewContainer.cropper('getData'),
                                ...
                                result = server + '&needOperate=' + (function () {
                                    ...
                                    Info.type = 'truncate';
                                    arr.push(Info);
                                    if(attrs.selfDefined){
                                        var selfDefinedObj={type:'scale'}
                                        selfDefinedObj.width=attrs.selfDefinedWidth || 100
                                        selfDefinedObj.height=attrs.selfDefinedHeight || 100
                                        arr.push(selfDefinedObj);
                                    }
                        ...
                        $scope.getResult = function ($event) {
                            ...
                            var imageLeng = instance.getFiles().length;
                            if (imageLeng === 0) {
                                HB_dialog.warning('提示', '请选择图片');
                                return false;
                            }
