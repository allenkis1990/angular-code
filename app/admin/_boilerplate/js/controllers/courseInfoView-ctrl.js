define(function () {
    'use strict';
    return ['$scope', 'selectCourseCenterService', '$stateParams', '$state',
        function ($scope, selectCourseCenterService, $stateParams, $state) {
            $scope.model = {
                selectIndex: 0,
                reviews: [],
                course: {},
            ...
            function findLessonInfo () {
                selectCourseCenterService.findLessonInfo($stateParams.courseId).then(function (data) {
                    if (data.status) {
            ...
            function findCourseReviewPage (add) {
                selectCourseCenterService.findCourseReviewPage($scope.model.pageNo, $scope.model.pageSize, $stateParams.courseId).then(function (data) {
                    if (data.status) {
                        if (add) {
