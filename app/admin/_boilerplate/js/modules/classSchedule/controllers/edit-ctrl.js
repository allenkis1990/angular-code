        ...
        function ($scope, classScheduleService, $state, $stateParams, $timeout, $q, HB_dialog, HB_notification, hbUtil) {
            ...
            $scope.$watch('model.resourceUploader', function (newValue) {
                ...
                if (!isBlank(newValue)) {
                    ...
                    var itemTemplateId = $scope.model.itemTemplateList[$scope.model.currentIndex].id;
                    classScheduleService.addPlanResourceToPlanItemTemplate(itemTemplateId, resource).then(function (response) {
                        if (response.status) {
                            HB_dialog.success('提示', "上传成功");
            ...
            $scope.model = {
                ...
                //以下为教师相关Model
                selectTeacherType: 1, //0：统一老师授课，1：每个课程独立老师授课
                selectTeacherInfo: {},
                teacherList: [],
                //以下为附件相关
                resType: 0,
            ...
            $scope.events = {
                ...
                submitItemTemplate: function (e) {
                    ...
                    classScheduleService.addPlanItemTemplate($.extend({
                        planTemplateId: $stateParams.id,
                        sort: ''
                    }, $scope.model.itemTemplateDto)).then(function (response) {
                        if (response.status) {
                ...
                canDeleteItemTemplate: function (e, dataItem) {
                    console.log('planItemTemplateId: ', dataItem.id);
                    if ($scope.model.itemTemplateList.length == 1) {
                        HB_dialog.error('提示', "课程表至少需要有1门课程，无法删除");
                        return;
                    }
                    classScheduleService.getItemTemplateCountByTemplateIds({
                        planTemplateId: $stateParams.id
                    }).then(function (response) {
                        if (response.status) {
                            var count = response.info;
                            if (count == -1 || count == 1) {
                                HB_dialog.error('提示', "课程表至少需要有1门课程，无法删除");
                                return;
                            } else {
                                $scope.events.deleteItemTemplate(e, dataItem);
                            }
                        } else {
                            HB_dialog.error('提示', response.info);
                        }
                ...
                deleteItemTemplate: function (e, dataItem) {
                    console.log('planItemTemplateId: ', dataItem.id);
                    $scope.globle.confirm("提示", '您正在删除课程，删除后无法恢复，是否确定删除？', function (dialog) {
                        classScheduleService.deletePlanItemTemplate({
                            id: dataItem.id
                        }).then(function (response) {
                            dialog.doRightClose();
                            if (response.status) {
                ...
                updateItemTemplate: function (e) {
                    ...
                    $scope.model.itemTemplateDto.itemResourceList = [];
                    classScheduleService.updatePlanItemTemplate($scope.model.itemTemplateDto).then(function (response) {
                        if (response.status) {
                            HB_dialog.success('提示', "操作成功");
                ...
                submitTemplate: function (e) {
                    ...
                    classScheduleService.updatePlanTemplate({
                        id: $stateParams.id,
                ...
                deleteSelectedCourse: function (e, index) {
                    var planItemTemplateId = $scope.model.itemTemplateList[$scope.model.currentIndex].id;
                    var planResourceTemplateId = $scope.model.courseSelectedList[index].id;
                    console.log("planItemTemplateId: ", planItemTemplateId);
                    console.log("planResourceTemplateId: ", planResourceTemplateId);
                    $scope.globle.confirm("提示", '确定删除？', function (dialog) {
                        classScheduleService.deletePlanResourceToPlanItemTemplate({
                            planItemTemplateId: planItemTemplateId,
                            planResourceTemplateId: planResourceTemplateId
                        }).then(function (response) {
                            dialog.doRightClose();
                            if (response.status) {
                                HB_dialog.success('提示', "操作成功");
                ...
                reNameConfirm: function (e, index) {
                    classScheduleService.updateResourceTemplate({
                        resourceTemplateId: $scope.model.attachmentList[index].id,
                        name: $scope.model.attachmentList[index].name
                    }).then(function (response) {
                        if (response.status) {
                ...
                deleteAttachment: function (e, index) {
                    var planItemTemplateId = $scope.model.itemTemplateList[$scope.model.currentIndex].id;
                    var planResourceTemplateId = $scope.model.attachmentList[index].id;
                    console.log("planItemTemplateId: ", planItemTemplateId);
                    console.log("planResourceTemplateId: ", planResourceTemplateId);
                    $scope.globle.confirm("提示", '确定删除？', function (dialog) {
                        classScheduleService.deletePlanResourceToPlanItemTemplate({
            ...
            (function () {
                ...
                result.push('<button type="button" class="table-btn" ng-click="events.addResourceWindow($event,dataItem.index-1)" has-permission="/classSchedule/UrlModifyPlanResourceTemplate">编辑参考资料</button>');
                ...
                result.push('<button type="button" class="table-btn" ng-click="events.canDeleteItemTemplate($event,dataItem)" has-permission="/classSchedule/UrlDeletePlanItemTemplate">删除</button>');
                result.push('</td>');
