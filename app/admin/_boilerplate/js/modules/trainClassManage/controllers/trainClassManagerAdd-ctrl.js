define(function () {
    'use strict';
    return ['$scope', 'trainClassManageService', '$state', 'kendo.grid', '$interval', '$timeout',
        function ($scope, trainClassManageService, $state, kendoGrid, $interval, $timeout) {
            ...
            $scope.events = {
                ...
                /**
                 *    e.preventDefault();
                 if ($scope.trainClassAssessValidate.$valid && $scope.model.savetrainClassAssess && $scope.model.examQuantityValidateDate && $scope.model.completeCourseQuantityValidateDate) {
                    $scope.model.trainClassAssess.trnId = $scope.model.trainClass.id;
                    $scope.model.savetrainClassAssess = false;
                    trainClassManageService.releaseTrainClass($scope.model.trainClassAssess).then(function (data) {
                        if (data.status) {
                            $scope.model.step += 20;
                            $scope.time = 10;
                ...
                saveTrainClassInfo: function (e) {
                    e.preventDefault();
                    if ($scope.trainClassValidate.$valid && $scope.model.saveInfo && $scope.model.validateDate && $scope.model.validateNumber) {
                        setTime();
                        if ($scope.model.notNumber) {
                            $scope.model.trainClass.registrationNumber = 0;
                        }
                ...
                checkCompleteCourseQuantity: function (data) {
                    if (data == null || data == '') {
                        ...
                        if (lessonNum < data) {
                            $scope.model.examQuantityValidateDate = false;
                            $scope.globle.alert('错误', '通过课程數量必须小于等于课程总数' + lessonNum + '门!');
                        } else {
                            if ($scope.model.passExam) {
                                var temp = $scope.model.trainClassAssess.passExamQuantity;
                                if (temp == null || temp == '') {
...
//设置报名的时间
            ...
            function init () {
                $scope.model.trainClass = null;
                $scope.validateParams = {
                    id: ''
                };
                //$scope.model.trainClass.beginTime=null;
                //$scope.model.trainClass.endTime=null;
                //$scope.model.trainClass.registrationStartTime=null;
                //$scope.model.trainClass.registrationEndTime=null;
                $scope.model.notNumber = false;
                $scope.model.validateNumber = false;
                ...
                //== 选择课程步骤
                $scope.model.lessonDimension = 1;// 选课维度; 1: 课程分类; 2: 能力项
                $scope.model.trainClass = {
                    registrationType: 0,
                    registerMode: 1
                };
                ...
                $scope.model.savetrainClassAssess = true;
                $scope.model.completeCourseQuantityValidateDate = true;
                $scope.model.examQuantityValidateDate = false;
