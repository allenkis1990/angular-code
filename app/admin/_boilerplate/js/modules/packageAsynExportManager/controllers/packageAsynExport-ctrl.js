define(function () {
    ...
    return ['$scope', 'HB_notification', 'hbUtil', '$http', 'questionService', 'kendo.grid', function ($scope, HB_notification, hbUtil, $http, questionService, kendoGrid) {
        $scope.model = {
            ...
            queryParam: {
                groupType: 'asynchExportPackageInfo', //异步任务组名
                startDate: null,
                endDate: null,
                status: '-1'
            },
        ...
        (function () {
            ...
            result.push('<td>');
            result.push('<button type="button" has-permission="packageExportManager/queryLog" class="table-btn"  ng-click="events.view($event,dataItem)" >查看日志</button>');
            result.push('<button type="button" has-permission="packageExportManager/exportpackage"  class="table-btn" ng-if="#: group==\'asynchExportPackageInfo\' #" ng-click="events.downloadLogExcel($event,dataItem)" #: log.status ==\'executed\' && log.result &&log.result.success?\'\':\'disabled\'#>下载导出文件</button>');
            result.push('</td>');
            ...
            grid: {
                options: {
                    ...
                    columns: [
                        ...
                        {field: 'log.result.result.operator', title: '操作人账号', sortable: false, width: 100},
