define(function () {
    'use strict';
    return ['$rootScope','$scope', '$stateParams', 'HB_notification', 'hbUtil', '$http', 'questionService', 'kendo.grid',
        function ($rootScope, $scope, $stateParams, HB_notification, hbUtil, $http, questionService, kendoGrid) {
        $scope.model = {
            page: {
                pageSize: 10,
                pageNo: 1
            ...
            queryParam: {
                groupType: 'asynchImportStudent',// $stateParams.groupType||"asynchImportStudent", //异步任务组名
                startDate: null,
                endDate: null,
                status: '-1'
            },
        ...
        (function () {
            ...
            result.push('<td>');
            result.push('<button type="button" class="table-btn" has-permission="importStudentTask/queryLog"  ng-click="events.view($event,dataItem)" >查看日志</button>');
            result.push('<button type="button"   class="table-btn" has-permission="importStudentTask/exportUser"  ng-if="#: group==\'asynchImportStudent\' #" ng-click="events.downloadLogExcel($event,dataItem,2)" #: log.status ==\'executed\' && log.result &&log.result.success&&log.result.result.failUrl?\'\':\'disabled\'#>下载错误数据文件</button>');
            result.push('<button type="button"   class="table-btn" ng-if="#: group==\'DISTRIBUTOR_OPEN\' #" ng-click="events.downloadLogExcel($event,dataItem,2)" #: log.status ==\'executed\' && log.result &&log.result.success&&log.result.result.failUrl?\'\':\'disabled\'#>下载错误数据文件</button>');
            result.push('</td>');
                                ...
                                data: function (e) {
                                    var temp = {
                                        page: {
                                            ...
                                            unitId: $scope.model.queryParam.unitId
                                        }
                                    };
