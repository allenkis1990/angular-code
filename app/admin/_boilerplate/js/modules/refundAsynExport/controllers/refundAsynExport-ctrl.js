define(function (refundAsynExport) {
    'use strict';
    return {
        indexCtrl: ['$scope', 'HB_notification', 'hbUtil', '$http', 'questionService', 'kendo.grid', function ($scope, HB_notification, hbUtil, $http, questionService, kendoGrid) {
            ...
            (function () {
                ...
                /*has-permission="refundAsynExport/view"*/
                result.push('<button type="button"  class="table-btn" has-permission="refundAsynExport/download" ng-if="#: group==\'REFUND_ORDER\' #" ng-click="events.downloadLogExcel($event,dataItem)" #: log.status ==\'executed\' && log.result &&log.result.success?\'\':\'disabled\'#>导出全部数据</button>');
                result.push('</td>');
                /*has-permission="refundAsynExport/download"*/
