define(function () {
    ...
    return ['$scope', 'HB_notification', 'hbUtil', '$http', 'questionService','kendo.grid', function ($scope, HB_notification, hbUtil, $http, questionService,kendoGrid) {
        $scope.model = {
            ...
            queryParam: {
                groupType: "UNIT_USER_IMPORT", //异步任务组名
                startDate: null,
                endDate: null,
                status: "-1"
            },
        ...
        (function () {
         ...
         /*   result.push('<td>');
            ...
            result.push('<button type="button"  has-permission="peopleManage/downloadLogExcel"  class="table-btn" ng-if="#: group==\'UNIT_USER_IMPORT\' #" ng-click="events.downloadLogExcel($event,dataItem,2)" #: log.status ==\'executed\' && log.result &&log.result.success&&log.result.result.failUrl?\'\':\'disabled\'#>下载错误数据文件</button>');
            result.push('</td>');
                    ...
                    dataSource: {
                        transport: {
                            read: {
                                ...
                                url: "/web/admin/asynJob/findAsynTaskLogByCurrentUser",
                                data: function (e) {
                                    var temp = {
                                        page: {
                                            pageNo: e.page,
