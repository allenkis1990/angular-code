define(function () {
    'use strict';
    return ['$scope', 'questionService', 'courseWareManagerService', '$stateParams', '$state',
        function ($scope, questionService, courseWareManagerService, $stateParams, $state) {
            ...
            courseWareManagerService.findCourseWare($stateParams.courseWareId).then(function (data) {
                if (data.status) {
                    ...
                    $scope.timeLength = data.info.timeLength;
                    $scope.model.courseWare = data.info;
                    $scope.validateParams.courseWareId = data.info.id;
                    $scope.model.courseWare.id = data.info.id;
                    $scope.model.courseWare.isUsable = '' + data.info.isUsable;
                    $scope.model.hour = parseInt($scope.timeLength / 3600);
                    $scope.model.minute = parseInt(($scope.timeLength % 3600) / 60);
                    $scope.model.second = parseInt($scope.timeLength - $scope.minute * 60 - $scope.hour * 3600);

                    var courseWare = {
                        record: true,
                        fileName: data.info.expandData,
                        type: data.info.type
                    };
                    $scope.model.coursewareList.push(courseWare);
                }
            });
            ...
            $scope.events = {
                ...
                modifiedShow: function (index) {
                    ...
                    if (that.questionType === '判断题') {
                         ...
                         */
                        var popOptions = that.options.split(' ');
                        var answer = that.answer.split(' ', that.answer.split(' ').length - 1);
                        $scope.model.questionCount = popOptions.length - 1;
                        $scope.model.question.configurationItems = [];
                        $scope.model.optionIndex = 0;
                        var keepGoing = true;
                ...
                updateCourseWare: function (e) {
                    if ($scope.courseWareValidate.$valid) {
                        ...
                        $scope.model.courseWare.expandData = $scope.model.coursewareList[0].fileName;
                        ...
                        courseWareManagerService.updateCourseWare($scope.model.courseWare).then(function (data) {
                            if (data.status) {
                ...
                getTypeInfo: function (dataItem) {
                    courseWareManagerService.hasChild(dataItem.id).then(function (data) {
                        if (!data.info) {
                            $scope.model.courseWare.cwyName = dataItem.name;
