define(function () {
    'use strict';
    return {
        indexCtrl: ['$scope', 'HB_notification', 'hbUtil', '$http', 'reconciliationViewsServices', 'kendo.grid', function ($scope, HB_notification, hbUtil, $http, reconciliationViewsServices, kendoGrid) {
            $scope.model = {
                page: {
                    pageSize: 10,
                    pageNo: 1
                ...
                queryParam: {
                    groupType: 'RECONCILIATION', //异步任务组名
                    startDate: null,
                    endDate: null,
                    status: '-1'
                },
            ...
            $scope.events = {};
            reconciliationViewsServices.downloadTemplate().then(function (data) {
                if (data.status) {
            ...
            (function () {
                ...
                result.push('<td>');
                result.push('<button type="button" class="table-btn" has-permission="orderAsynchExport/queryLog" ng-click="events.view($event,dataItem)" >查看日志</button>');
                result.push('<button type="button" has-permission="orderAsynchExport/download" class="table-btn" ng-if="#: group==\'RECONCILIATION\' #" ng-click="events.downloadLogExcel($event,dataItem)" #: log.status ==\'executed\' && log.result &&log.result.success?\'\':\'disabled\'#>下载导出的订单</button>');
                result.push('</td>');
