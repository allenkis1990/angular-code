...
define(function () {
    ...
    var controller = ['$scope', '$log', 'paperService', 'KENDO_UI_GRID', '$state',

        function ($scope, $log, paperService, KENDO_UI_GRID, $state) {
            ...
            $scope.model = {
                ...
                examRound: {
                    ...
                    examTimeLength: paper.timeLength,
                    examModeType: '1',
                    displayType: '1',
            ...
            $scope.model.examPaperName = paper.name;
            $scope.model.totalScore = paper.totalScore;
            $scope.show = {
            ...
            $scope.events = {
                ...
                release: function () {
                    ...
                    paperService.releasePaper($scope.model.examRound).then(function (data) {
                        $scope.submitNow = false;
                        if (!data.status) {
                            $scope.globle.alert('发布失败!', data.info);
                        } else {
                            $state.go('states.practicePaperConfig.continue', {comeForm: $scope.$stateParams.comeForm});
                        }
                    });
                },
                ...
                startTimeChange: function () {
                    if ($scope.model.examRound.examModeType === '2' && $scope.model.examRound.examTimeLength != null) {
                        var beginTime = new Date($scope.model.examRound.beginTime).getTime();
                        var examTimeLength = $scope.model.examRound.examTimeLength * 1000 * 60;
                        var endTime = beginTime + examTimeLength;
                        var endDate = new Date(endTime);
                        var nowStr = endDate.format('yyyy-MM-dd hh:mm:ss');
                        $scope.model.examRound.endTime = nowStr;
                    }
                }
            };
