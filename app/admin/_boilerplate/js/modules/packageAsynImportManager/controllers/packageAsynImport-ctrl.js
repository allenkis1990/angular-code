define(function () {
    ...
    return ['$scope', 'HB_notification', 'hbUtil', '$http', 'questionService', 'kendo.grid', function ($scope, HB_notification, hbUtil, $http, questionService, kendoGrid) {
        $scope.model = {
            ...
            queryParam: {
                groupType: 'asynchImportPackageInfo', //异步任务组名
                startDate: null,
                endDate: null,
                status: '-1'
            },
        ...
        (function () {
            ...
            result.push('<td>');
            result.push('#: log.result == null ? "未知" : log.result.result.operator != null ? log.result.result.operator : "未知" #');
            result.push('</td>');
            ...
            result.push('<td>');
            result.push('<button type="button" has-permission="packageImportManager/queryLog" class="table-btn"  ng-click="events.view($event,dataItem)" >查看日志</button>');
            result.push('<button type="button" has-permission="packageImportManager/exportRightFile" class="table-btn" ng-if="#: group==\'asynchImportPackageInfo\' #" ng-click="events.downloadLogExcel($event,dataItem,1)" #: log.status ==\'executed\' && log.result &&log.result.success&&log.result.result.resultUrl?\'\':\'disabled\'#>下载正确数据文件</button>');
            result.push('<button type="button" has-permission="packageImportManager/exportErrorFile"  class="table-btn" ng-if="#: group==\'asynchImportPackageInfo\' #" ng-click="events.downloadLogExcel($event,dataItem,2)" #: log.status ==\'executed\' && log.result &&log.result.success&&log.result.result.failUrl?\'\':\'disabled\'#>下载错误数据文件</button>');
            result.push('</td>');
            ...
            grid: {
                options: {
                    ...
                    columns: [
                        ...
                        {field: 'log.result.result.startDate', title: '任务处理时间', sortable: false, width: 140},
                        {field: 'log.result.result.endDate', title: '任务结束时间', sortable: false, width: 140},
                        {field: 'log.result.result.operator', title: '操作人账号', sortable: false, width: 90},
                        {field: 'log.result.result.rowSum', title: '处理条数', sortable: false, width: 70},
