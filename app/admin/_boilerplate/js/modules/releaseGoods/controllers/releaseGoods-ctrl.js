        ...
        indexCtrl: ['$scope', 'HB_dialog', '$q', '$http', 'hbUtil', '$state', 'TabService', '$stateParams', 'releaseGoodsServices', 'easyKendoDialog', '$timeout', 'HB_notification', function ($scope, HB_dialog, $q, $http, hbUtil, $state, TabService, $stateParams, releaseGoodsServices, easyKendoDialog, $timeout, HB_notification) {
            ...
            $scope.$watch('model.submitType', function (nv) {
                if (nv) {
                    ...
                    if (nv === 'trainingClass') {
                        ...
                        //如果培训方案类型为自主选课 学习方式tab里的考试、兴趣课、练习tab隐藏
                        angular.forEach($scope.model.studyTypeArr, function (item) {
                            //item.show=true;
            ...
            $scope.$watch('model.uploader', function (newVal) {
                if (newVal) {
                    ...
                    //如果上传图片需要裁剪后的图片要用newVal.convertResult.url 不用裁剪的话就是newVal.newPath
            ...
            $scope.model = {
                ...
                categoryType: 'TRAINING_CLASS_GOODS',//学习方案形式的类目培训班：TRAINING_CLASS_GOODS 自主选课:COURSE_SUPERMARKET_GOODS
                modViewList: [],
                hasEditPop: false,
            ...
            $scope.events = {
                ...
                /*lookHasNotPopCourseList:function(){
                    if(validateIsNull($scope.submitData.courseLearning.ruleId)){
                        HB_dialog.warning('警告', '请先配置课程学习内容，否则无法校验有弹窗题');
                        return false;
                    }

                    //$state.go('states.hasnotPopQuestionCourseList');
                    var location=window.location;
                    //console.log(location);
                    window.open(location.pathname+'#'+'/hasnotPopQuestionCourseList/'+$scope.submitData.courseLearning.ruleId);
                },*/
                ...
                toggleEnableRedoPracticePaper: function (e) {
                    if (e.target.checked) {
                        $scope.submitData.practicePaperConfig.enableRedoPracticePaper = true;
                    } else {
                        $scope.submitData.practicePaperConfig.enableRedoPracticePaper = false;
                    }
                },

                addPracticePaperAssessConfig: function (e) {
                        $scope.submitData.practicePaperConfig.enableAfterCourseLearning = true;
                        $scope.submitData.practicePaperConfig.enableRedoPracticePaper = true;
                },

                deletePracticePaperAssessConfig: function (e) {
                        $scope.submitData.practicePaperConfig.enableRedoPracticePaper = false;
                ...
                preview: function (e, item) {
                    ...
                    $http.get('/web/admin/certifiedTemplate/getCertifiedPreview?id=' + item.id).success(function (data) {
                        if (data.status) {
                ...
                openCourseRuleDialog: function () {
                    $scope.courseRuleWindow = easyKendoDialog.content({
                        templateUrl: '@systemUrl@/views/releaseGoods/edit-courseRule-dialog.html',
                        width: 800,
                        title: false
                    }, $scope);
                },
                ...
                submit: function () {
                    ...
                    if ($scope.model.submitType === 'trainingClass') {
                        ...
                        if ($scope.submitData.price % 0.01 !== 0) {
                            HB_dialog.warning('警告', '培训班价格必须是0.01的倍数');
                            return false;
                        }
                        ...
                        //单测验
                        if ($scope.model.studyTypeArr[5].ischecked && !$scope.model.studyTypeArr[3].ischecked && $scope.submitData.practicePaperConfig.addPracticePaperAssess === 'true') {
                        ...
                        if ($scope.model.studyTypeArr[5].ischecked && $scope.model.studyTypeArr[3].ischecked) {
                            ...
                            if (validateIsNull($scope.submitData.practicePaperConfig.addPracticePaperAssess === 'true' && $scope.model.goodsTrainingConfigRequire.practicePaperPassScore)) {
                                HB_dialog.warning('警告', '请填写课程课后测验成绩');
                                return false;
                            }

                            if (validateIsNaN($scope.submitData.practicePaperConfig.addPracticePaperAssess === 'true' && $scope.model.goodsTrainingConfigRequire.practicePaperPassScore)) {
                                HB_dialog.warning('警告', '课程课后测验成绩必须为数字');
                                return false;
                            }

                            if ($scope.submitData.practicePaperConfig.addPracticePaperAssess === 'true' && $scope.model.goodsTrainingConfigRequire.practicePaperPassScore <= 0) {
                                HB_dialog.warning('警告', '课程课后测验成绩必须大于0');
                                return false;
                            }

                            if ($scope.submitData.practicePaperConfig.addPracticePaperAssess === 'true' && $scope.model.goodsTrainingConfigRequire.practicePaperPassScore > $scope.model.practiceTotalScore) {
                                HB_dialog.warning('警告', '课程课后测验成绩不能大于测验总分');
                                return false;
                            }
                        ...
                        console.log('enableRedoPracticePaper:' + $scope.submitData.practicePaperConfig.enableRedoPracticePaper)
                        console.log('enableAfterCourseLearning:' + $scope.submitData.practicePaperConfig.enableAfterCourseLearning)
                        ...
                        var submitParams = {
                            ...
                            skuPropertyList: $scope.skuParams.skuPropertyList,
                            courseLearning: $scope.submitData.courseLearning,
                            practicePaperConfig: !$scope.model.studyTypeArr[5].ischecked ? undefined : $scope.submitData.practicePaperConfig,
                            popupQuestionConfig: !$scope.model.studyTypeArr[3].ischecked ? undefined : $scope.submitData.popupQuestionConfig,
            ...
            function initParams () {
                ...
                if (!$scope.submitData.courseLearning.poolList) {
                    $scope.submitData.courseLearning.poolList = [];
                }
            ...
            //发布商品接口 发布成功后关闭发布页面跳去商品列表页
            function submit (params, methodName) {
                $scope.submitAble = true;
            ...
            function validateReleaseBas (name) {
                ...
                angular.forEach($scope.skuParams.skuPropertyList, function (item) {
                    ...
                    console.log(item.propertyIdCode);
                    console.log(item.valueCode);
                    console.log((item.propertyIdCode === 'trainingSubject' && item.valueCode === 'profession' && $scope.model.categoryType === 'COURSE_SUPERMARKET_GOODS' && requireSkuLen === 1) || requireSkuLen === 2);
                });


                console.log($scope.model.categoryType);
                console.log(requireSkuLen);
                if (!skuMark) {
            ...
            //如果学时定价被配置过了 下次再配置另一个学时定价的时候之前配置过的显灰
            function disabledDo () {
            ...
            //如果独立定价被配置过了 下次再配置另一个学时定价的时候之前配置过的显灰
