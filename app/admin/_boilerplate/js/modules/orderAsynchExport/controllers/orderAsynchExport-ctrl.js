define(function (orderAsynchExport) {
    'use strict';
    return {
        indexCtrl: ['$scope', 'HB_notification', 'hbUtil', '$http', 'questionService', 'kendo.grid', function ($scope, HB_notification, hbUtil, $http, questionService, kendoGrid) {
            ...
            (function () {
                ...
                result.push('<button type="button" has-permission="orderAsynchExport/download" class="table-btn" ng-if="#: group==\'ORDER\' #" ng-click="events.downloadLogExcel($event,dataItem)" #: log.status ==\'executed\' && log.result &&log.result.success?\'\':\'disabled\'#>下载导出的订单</button>');
                result.push('</td>');
