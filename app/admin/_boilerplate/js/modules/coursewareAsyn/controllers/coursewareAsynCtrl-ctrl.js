define(function () {
    'use strict';
    return ['$scope', 'HB_notification', 'hbUtil', '$http', 'questionService', 'kendo.grid', function ($scope, HB_notification, hbUtil, $http, questionService) {
        $scope.model = {
            page: {
                pageSize: 10,
                pageNo: 1
            ...
            queryParam: {
                groupType: 'coursewareAsyn', //默认查看试题导入的日志
                startDate: null,
                endDate: null,
                status: '-1'
            },
        ...
        (function () {
            ...
            result.push('<td>');
            result.push('<button type="button" class="table-btn" has-permission="popQuestionImport/findAsynRecordPage" ng-click="events.view($event,dataItem)" >查看日志</button>');
            result.push('<button type="button" class="table-btn" has-permission="popQuestionImport/exportEntireAsynRecord" ng-if="#: group==\'coursewareAsyn\' #" ng-click="events.downloadLogExcel($event,dataItem,1)" #: log.status ==\'executed\' && log.result &&log.result.success && log.result.result.result!=\'ERROR\'?\'\':\'disabled\'#>导出全部数据</button>');
            result.push('<button type="button" class="table-btn" has-permission="popQuestionImport/exportFailAsynRecord" ng-if="#: group==\'coursewareAsyn\' #" ng-click="events.downloadLogExcel($event,dataItem,2)" #: log.status ==\'executed\' && log.result &&log.result.success && (log.result.result.result==\'PART_SUCCESS\' || log.result.result.result==\'FAIL\')?\'\':\'disabled\' #>导出失败数据</button>');
            result.push('</td>');
