define ( function ( distributionQuery ) {
    "use strict";
    return {
        indexCtrl: ["$scope",'kendo.grid', 'hbUtil', 'HB_dialog', 'TabService', '$http', '$timeout', '$rootScope', '$state', 'unitAdminServices','distributionQueryService','HB_notification',
            function ( $scope, kendoGrid,  hbUtil, HB_dialog, TabService, $http, $timeout, $rootScope, $state, unitAdminServices,distributionQueryService,HB_notification) {
                ...
                $scope.events = {
                    ...
                    openwindow: function (item){
                        ...
                        HB_dialog.contentAs ( $scope, {
                            ...
                            templateUrl: 'views/customer/distributionQuery/remark.html'
                        } );
                    },
                ...
                (function () {
                    ...
                    result.push ( '<div class="goods-info " >' +
                        '<p  ng-if="#: packageContent.batchNum!==null#">批次号：#: packageContent.batchNum #</p>' +
                        '<p  ng-if="#: packageContent.traceSuccessTime!==null#">交易成功时间：#: packageContent.traceSuccessTime #</p>' +
                        '<p  ng-if="#: packageContent.goodNameList!==null#" ng-repeat=" goodNameList in  dataItem.packageContent.goodNameList ">b{{goodNameList}};  </p>' +
                        ' <div   ng-if="#: packageContent.orderBills!==null#"  class="tips-box mt5 mb5" ng-repeat=" orderBills in  dataItem.packageContent.orderBills ">【b{{orderBills.name}}】' +
                        '<span ng-if="#: packageContent.orderBills.title!==null#">发票抬头：b{{orderBills.title}}</span>' +
                        '<span class="ml20"  ng-if="#: packageContent.orderBills.count!==null#">数量：b{{orderBills.count}} </span>' +
                        '<span  class="ml20" ng-if="#: packageContent.orderBills[0].orderBillNo===null#">发票号：未打印 </span><span class="ml20"  ng-if="#: packageContent.orderBills[0].orderBillNo!==null#">发票号：b{{orderBills.orderBillNo}} </span></div></div>' );
                    ...
                    result.push ( '<p  ng-if="#: carrierName!==null #">快递公司：#: carrierName # </p>' +
                        ...
                        '<a after-copy="events.afterCopy(dataItem)" copy-man ng-if="#: waybillNo!==null #" class="txt-b" data-clipboard-text="b{{dataItem.waybillNo}}">' +
                        '复制运单号并查询</a>' );
                    result.push ( '</td>' );
                    ...
                    result.push ( '<span ng-if=\"#: state==="4" #\" class="txt-r">拣货中</span>' );/*<a href="" class="t-btn btn btn-mini">备注</a>*/
                    ...
                    result.push ( '<td class="op"  >');
         result.push ( '<a href="javascript:void(0)"  class="t-btn btn btn-mini" ng-click=\"events.openwindow(dataItem)\"  has-permission="customer/remarkPackage"  >备注</a>' );
                    result.push ( '</td>' );
                ...
                $scope.deliveryQueryGrid = {
                    options: {
                                ...
                                read: {
                                    ...
                                    data       : function ( e ) {
                                        ...
                                        var temp = {
                                            ...
                                            batchNum           : $scope.deliveryQueryParam.orderNo,
                                            batchQueryTag     :1
                            ...
                            schema       : {
                                ...
                                parse: function ( response ) {
                                   ...
                                   angular.forEach (response.info, function ( item ) {
                                       ...
                                       /* console.log(item)
                                         $scope.model.remark =item.remark;*/
                                       $scope.model.goodNameList =item.packageContent.goodNameList ;
                        ...
                        columns    : [
                            ...
                            { field: "goodsInfo", title: "包裹物品", sortable: false, width: 300 },
