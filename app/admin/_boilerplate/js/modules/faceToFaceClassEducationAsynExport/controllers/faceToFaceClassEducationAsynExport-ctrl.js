define(function () {
    ...
    return ['$scope', 'HB_notification', 'hbUtil', '$http', 'questionService', 'kendo.grid', function ($scope, HB_notification, hbUtil, $http, questionService, kendoGrid) {
        $scope.model = {
            ...
            queryParam: {
                groupType: 'EXPORT_USER_INFO_IN_CLASS', //异步任务组名
                startDate: null,
                endDate: null,
                status: '-1'
            },
        ...
        $scope.events = {
            ...
            downloadFailLogExcel: function (e, dataItem) {
        ...
        (function () {
            ...
            result.push('<button type="button" class="table-btn" ng-if="#: group==\'EXPORT_USER_INFO_IN_CLASS\' || group==\'EXPORT_ACCOMMODATIONS\'#" ng-click="events.downloadLogExcel($event,dataItem)" #: log.status ==\'executed\' && log.result &&log.result.success?\'\':\'disabled\'#>下载导出文件</button>');
            result.push('<button type="button" class="table-btn" ng-if="#: group==\'IMPORT_ACCOMMODATIONS\'#" ng-click="events.downloadFailLogExcel($event,dataItem)" #: log.status ==\'executed\' && log.result &&log.result.success&&log.result.result.failUrl?\'\':\'disabled\'#>下载错误数据文件</button>');
            result.push('</td>');
