...
define(function () {
    ...
    var controller = ['kendo.grid', 'KENDO_UI_TREE', '$scope', '$log', 'paperService', 'KENDO_UI_GRID', '$state', 'HB_notification',
        ...
        function (kendoGrid, KENDO_UI_TREE, $scope, $log, paperService, KENDO_UI_GRID, $state, HB_notification) {
            ...
            var removeQuestions = [];
            var backUpData = {};
            $scope.$watch('model.paper.configType', function () {
                if ($scope.model.paper.configType === '1') {
                    $scope.model.paper.configurationItemDtos = [];
                }
            });
            ...
            $scope.events = {
                ...
                continueAdd: function () {
                    if ($scope.addPaperStep1Form.$valid) {
                        ...
                        if ($scope.model.paper.examTypeId == null || $scope.model.paper.examTypeId === '') {
                            $scope.globle.alert('提示', '请选择一个试卷分类');
                            return;
                        }
                ...
                savePaper: function (way) {
                    ...
                    if (way !== '2') {
                        ...
                        var count = 0;
                        _.forEach($scope.model.paper.configurationItemDtos, function (question) {
                            var score = question.totalScore * 1;
                            count += score;
                        });
                    ...
                    if ($scope.model.paper.configType === '1') {
                        $scope.model.paper.fixedConfigurationItemAddDtos = $scope.model.paper.configurationItemDtos;
                        paperService.createFixedPaper($scope.model.paper).then(function (data) {
                            if (!data.status) {
                                $scope.globle.alert('创建失败!', data.info);
                            } else {
                                if (way !== '2') {
                                    $state.go('states.paperConfig.addStep3', data.info);
                                } else {
                    ...
                    } else if ($scope.model.paper.configType === '3') {
                        var isCommit = true;
                        if ($scope.model.paper.randomType === '1') {
                            var libraryIds = [];
                            ...
                            paperService.findExamPaperQuestionCount(null, libraryIds).then(function (data) {
                                ...
                                if (isCommit) {
                                    ...
                                    $scope.model.paper.ratio = 'false';
                                    paperService.createSmartPaper($scope.model.paper).then(function (data) {
                                        if (!data.status) {
                                            $scope.globle.alert('创建失败!', data.info);
                                        } else {
                                            if (way !== '2') {
                ...
                initAddQuestion: function (questionType) {
                    ...
                    $scope.model.questionSearch = {
                        ...
                        mode: '-1',
                        librarys: null,
                        courseName: $scope.model.selectedCourseName
                    };
                    $scope.model.searchCourseName = $scope.model.selectedCourseName;
                ...
                saveQuestion: function () {
                    if ($scope.model.bugQuestions.name == null) {
                        $scope.globle.showTip('大题名称不能为空，或者大于30个汉字', 'error');
                        return;
                    }
                ...
                saveSmartPaperQuestion: function () {
                    if ($scope.smartPaperAddQuestionForm.$valid) {
                        ...
                        if (saveOrUpdate === 'update') {
                            ...
                            console.log($scope.model.paper.configurationItemDtos);
                        }
            ...
            (function () {
                ...
                result.push('<td>');
                result.push('<button class="table-btn" ng-if="!dataItem.isChoice" has-permission="paperConfig/confirmQuestionForAdd" ng-click="events.chooseQuestion(dataItem)">选择</button>');
                result.push('<button class="table-btn" ng-if="dataItem.isChoice" has-permission="paperConfig/confirmQuestionForAdd" ng-click="events.removeQuestion(\'#: id #\')">取消选择</button>');
                result.push('</td>');
...
// 填空题试题，构建表格的内容模板
            var fillQuestionGridRowTemplate = '';
            (function () {
                var result = [];
                result.push('<tr>');
                ...
                result.push('<td ng-if="#: answerCount!==null #">');
                result.push('#: answerCount #');
                result.push('</td>');
                ...
                result.push('</tr>');
                fillQuestionGridRowTemplate = result.join('');
            })();
