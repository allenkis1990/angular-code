define ( function (detail) {
    "use strict";
    return {
        indexCtrl: ["$scope",'$timeout','hbUtil','KENDO_UI_GRID', 'KENDO_UI_TREE', 'kendo.grid', 'batchManageServices', '$stateParams', '$http', '$q', 'HB_dialog', '$state', 'HB_notification','TabService','hbSkuService',
            function ( $scope,$timeout,hbUtil,KENDO_UI_GRID, KENDO_UI_TREE, kendoGrid,batchManageServices, $stateParams, $http, $q, HB_dialog, $state, HB_notification,TabService,hbSkuService) {
                    ...
                    commitOrder: function () {
                        batchManageServices.findBatchDetail({
                            ...
                            if($scope.model.batchInfo.people===0){
                                ...
                                $state.go('states.batchManage.batchOrder',{batchNo:$stateParams.batchNo}).then ( function () {
                                });
                            }
                        })
                    ...
                    addGoodsToBatchList:function(type){
                        ...
                        if(type===1){
                            batchManageServices.addGoodsToBatchList({
                                batchNo:$scope.model.batchInfo.no,
                ...
                (function () {
                    ...
                    /*  result.push ( '<button type="button" class="table-btn" has-permission="userManage/query" ng-click="events.view($event,dataItem)">查看</button>' );
              ...
              /*  batchManageServices.findUserBatchPage({
                ...
                batchManageServices.findBatchDetail({
                    ...
                    $scope.model.batchInfo = data.info;
                    if (data.info.test){
                        $scope.model.choosePeopleParam.loginInputType = 1;
                    }
