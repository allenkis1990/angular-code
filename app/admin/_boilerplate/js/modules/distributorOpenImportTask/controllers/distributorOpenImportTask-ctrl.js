define(function (distributorOpenImportTask) {
    'use strict';
    return {
        indexCtrl: ['$scope', 'HB_dialog', 'HB_notification', 'hbUtil', '$q', 'openImportTaskService', 'questionService', function ($scope, HB_dialog, HB_notification, hbUtil, $q, openImportTaskService, questionService) {
            $scope.model = {
                urlPrefix: '',
                upload: {},
                ...
                queryParam: {
                    groupType: 'DISTRIBUTOR_OPEN', //异步任务组名
                    startDate: null,
                    endDate: null,
                    status: '-1'
                }
            ...
            $scope.events = {
                ...
                chooseFile: function (e) {
                    ...
                    HB_dialog.contentAs($scope, {
                        ...
                        width: 500,
                        height: 270,
                        templateUrl: '@systemUrl@/views/distributorOpenImportTask/tmp-import.html',
                        sure: function (wow) {
                            var defer = $q.defer(),
                                promise = defer.promise;
                            if ($scope.model.upload.result) {
                                openImportTaskService.importOpenUser({filePath: $scope.model.upload.result.newPath}).then(function (data) {
                                    if (data.status) {
                                        HB_dialog.success('提示', '导入成功');
                                    } else {
            ...
            (function () {
                ...
                result.push('<td>');
                result.push('#: log.status == "executed"&&log.result.result!={}&&log.result.result!=null&&log.result.result.totalCount ? log.result.result.totalCount : "----"#');
                result.push('</td>');

                result.push('<td>');
                result.push('#: log.status == "executed"&&log.result.result!={}&&log.result.result!=null&&log.result.result.successCount ? log.result.result.successCount : "----"#');
                result.push('</td>');

                result.push('<td>');
                result.push('#: log.status == "executed"&&log.result.result!={}&&log.result.result!=null&&log.result.result.failCount ? log.result.result.failCount : "----"#');
                result.push('</td>');


                result.push('<td>');
                result.push('<button type="button" class="table-btn" ng-click="events.view($event,dataItem)" >查看日志</button>');
                result.push('<button type="button" class="table-btn" ng-if="#: group==\'DISTRIBUTOR_OPEN\' #" ng-click="events.downloadLogExcel($event,dataItem)" #: log.status =="executed" && log.result && log.result.success && log.result.result.fileUrl? "" : "disabled" #>下载导入失败名单</button>');
                result.push('</td>');
                ...
                grid: {
                    options: {
                        ...
                        columns: [
                            ...
                            {title: '处理总数', sortable: false, width: 100},
                            {title: '成功数', sortable: false, width: 100},
                            {title: '失败数', sortable: false, width: 100},
                            {
                                title: '操作', width: 250
                            }
                        ]
