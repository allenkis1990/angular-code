define ( function () {
    'use strict';
    return ['$scope', '$log', 'infoCategoryService', '$state', '$stateParams', 'TabService', function ( $scope, $log, infoCategoryService, $state, $stateParams, TabService ) {
        $scope.node   = {
        ...
        $scope.model = {
            ...
            unitInfoList : null//顶级单位列表
        };

        init ();
        function init() {
            infoCategoryService.isSuperAdmin().then(function(data){
                if(data.status){
                    $scope.model.isSuperAdmin = data.info;
                    if($scope.model.isSuperAdmin){//是超级管理员
                        infoCategoryService.getUserUnitRegionName().then(function(dataTemp){
                            $scope.model.regionName = dataTemp.info;
                        });
                        infoCategoryService.getTopUnitInfoList().then(function(dataInfo){
                            $scope.model.unitInfoList = dataInfo.info;
                            if(dataInfo.status){
                                infoCategoryService.getUserUnitId().then(function(dataTemp){
                                    $scope.model.condition.currentUnitId = dataTemp.info;
                                });
                            }else{
        ...
        var dataSource = new kendo.data.HierarchicalDataSource ( {
            transport: {
                read: function ( options ) {
                    ...
                    $.ajax ( {
                        url     : "/web/admin/infoCategory/findAllSubListByParent?parentId=" + id + "&status=1&unitId=" + $scope.model.condition.currentUnitId,
            ...
            dataSource: {
                transport: {
                    read: function ( e ) {
                        ...
                        infoCategoryService.findTreeListData ( $scope.model.condition ).then ( function ( result ) {
                            if ( result.status ) {
                                ...
                                $scope.globle.alert ( "提示", result.info );
                                return;
                            }
        ...
        $scope.events = {
            ...
            edit                      : function ( e, dateItem ) {
                e.stopPropagation ();
                infoCategoryService.findForUpdate ( dateItem.id ).then ( function ( data ) {
                    if ( data.status ) {
            ...
            save: function ( e ) {
                ...
                $scope.model.dto.regionPath = $scope.model.regionPath;
                $scope.model.dto.currentUnitId = $scope.model.condition.currentUnitId;
                infoCategoryService.save ( $scope.model.dto ).then ( function ( data ) {
                    if ( data.status ) {
            ...
            detail: function ( e, dateItem ) {
                e.stopPropagation ();
                infoCategoryService.findForDetail ( dateItem.id ).then ( function ( data ) {
                    if ( data.status ) {//parentName: null,name: null,description: null,status: null,newsCount: null
                        ...
                        $scope.model.detailInfo.unitId = data.info.unitId;
            ...
            goNewsNotice: function ( e, dataItem ) {
                ...
                $scope.globle.stateGo ( 'states.infoContent', '资讯内容', {
            ...
            dealStart: function ( e, item ) {
                ...
                $scope.globle.confirm (
                    ...
                    function ( dialog ) {
                        return infoCategoryService.start ( item.id ).then ( function ( response ) {
                            dialog.doRightClose ();
                            if ( response.status ) {
                                item.status = 1;
            ...
            dealStop: function ( e, item ) {
                e.stopPropagation ();
                infoCategoryService.checkStopAble ( item.id ).then ( function ( response ) {
                    if ( !response.status ) {
                        ...
                        $scope.globle.confirm (
                            ...
                            function ( dialog ) {
                                return infoCategoryService.stop ( item.id ).then ( function ( response ) {
                                    dialog.doRightClose ();
                                    if ( response.status ) {
                                        item.status = 0;
                                        $scope.globle.showTip ( response.info, 'success' );
            ...
            dealDelete: function ( e, item ) {
                e.stopPropagation ();
                infoCategoryService.checkDeleteAble ( item.id ).then ( function ( response ) {
                    if ( !response.status ) {
                        $scope.globle.alert ( '提示', response.info );
                        return;
                    } else {
                        $scope.globle.confirm (
                            '删除分类', "确定删除该分类吗？",
                            function ( dialog ) {
                                return infoCategoryService.deleteById ( item.id ).then ( function ( response ) {
                                    dialog.doRightClose ();
                                    if ( response.status ) {
                                        $scope.model.condition.parentId = "0";
                                        $scope.node.newsCategoryTreeList.dataSource.read ();
