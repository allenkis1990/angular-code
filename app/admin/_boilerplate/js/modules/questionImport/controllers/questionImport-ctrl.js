define(function () {
    ...
    return ['$scope', 'HB_notification', 'hbUtil', '$http', 'questionService', 'kendo.grid', function ($scope, HB_notification, hbUtil, $http, questionService, kendoGrid) {
        $scope.model = {
            ...
            queryParam: {
                groupType: 'questionImport', //默认查看试题导入的日志
                startDate: null,
                endDate: null,
                status: '-1'
            },
        ...
        $scope.events = {
            ...
            /**
             * 查看未发布对象详情  bylinf
             * @param releaseExamRoundId
             */
            personDetail: function (dataItem) {
                $scope.node.windows.teacherWindow.open();
                $scope.model.personQueryParam.examRoundId = dataItem.log.result.result.examRoundId;
                $scope.node.personGrid.pager.page(1);
            }
        };
        ...
        (function () {
            ...
            result.push('<button type="button" has-permission="questionImport/exportEntireAsynRecord" class="table-btn" ng-if="#: group==\'questionImport\' #" ng-click="events.downloadLogExcel($event,dataItem,1)" #: log.status ==\'executed\' && log.result &&log.result.success && log.result.result.result!=\'ERROR\'?\'\':\'disabled\'#>导出全部数据</button>');
            result.push('<button type="button" has-permission="questionImport/exportFailAsynRecord" class="table-btn" ng-if="#: group==\'questionImport\' #" ng-click="events.downloadLogExcel($event,dataItem,2)" #: log.status ==\'executed\' && log.result &&log.result.success && (log.result.result.result==\'PART_SUCCESS\' || log.result.result.result==\'FAIL\')?\'\':\'disabled\' #>导出失败数据</button>');
            result.push('</td>');
        ...
        $scope.ui = {
            ...
            grid: {
                options: {
                    ...
                    columns: [
                        {field: 'name', title: '任务名称', sortable: false},
                        ...
                        {field: 'log.username', title: '操作人', sortable: false, width: 150},
                        {field: 'log.result.success', title: '任务处理结果', sortable: false, width: 100},
                        {field: 'log.result.result.rowSum', title: '处理条数', sortable: false, width: 80},
                        {field: 'log.result.result.rowSuccess', title: '成功条数', sortable: false, width: 80},
                        {field: 'log.result.result.rowFail', title: '失败条数', sortable: false, width: 80},
                        {
                            title: '操作', width: 240
                        }
                    ]
                ...
                options: {
                    ...
                    dataSource: {
                        transport: {
                            read: {
                                contentType: 'application/x-www-form-urlencoded;charset=utf-8;',
                                url: '/web/admin/paper/getPersonNameByExamRoundId',
                                data: function (e) {
                                    var temp = {
                                        //page: {
                                        //    pageNo: $scope.model.personQueryParam.page.pageNo,
                                        //    pageSize: $scope.model.personQueryParam.page.pageSize
                                        //},
                                        ...
                                        examRoundId: $scope.model.personQueryParam.examRoundId
                                    };
                                    $scope.model.personQueryParam.page.pageNo = e.page;
                                    $scope.model.personQueryParam.page.pageSize = e.pageSize;
                                    delete e.page;
                                    delete e.pageSize;
                                    delete e.skip;
                                    delete e.take;
