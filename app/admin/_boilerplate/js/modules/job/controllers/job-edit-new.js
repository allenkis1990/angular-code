define(function () {
    'use strict';
    return ['$scope', 'jobService', '$state', 'kendo.grid', function ($scope, jobService, $state, kendoGrid) {
        ...
        editNewJob.utils = {
            ...
            reCalculateGradeLesson: function (cursor) {
                var lessonList = $scope.model.editNew.gradeList[cursor].lessonList;
            ...
            getJobGradeValidResult: function () {
                var gradeList = $scope.model.editNew.gradeList,
            ...
            refreshJobGradeLevel: function () {
                angular.forEach($scope.model.editNew.gradeList, function (grade, index) {
                    ...
                    if (!$scope.ui.lessonType) {
                        $scope.ui.lessonType = {
                            ...
                            select: function (e) {
                                ...
                                // 加载课程数量、课程分页
                                editNewJob.utils.loadLessonCounts();
            ...
            loadLessonCounts: function () {
                ...
                jobService.getPackageLessonCounts(gridParams).then(function (response) {
                    if (response.status) {
            ...
            initialLessonGrid: function () {
                $scope.ui.lessonGrid = {
                    ...
                    // 每个行的模板定义,
                    rowTemplate: kendo.template(editNewJob.uiTemplate.lessonGridRow()),
                    noRecords: {
                        template: '暂无数据'
                    },
                            ...
                            parse: function (response) {
                                ...
                                if (response.status) {
                                    var viewData = response.info,
                                        excludeLessonIdList = $scope.model.lessonGridParams.selectedLessonIdList;

                                    _.forEach(viewData, function (row) {
                                        row.isChoice = false;
                                        _.forEach(excludeLessonIdList, function (lessonId) {
                                            if (row.id === lessonId) {
                                                row.isChoice = true;
                                            }
                                        });
        ...
        $scope.events = {
            ...
            addGrade: function (e) {
                ...
                if (gradeList[size - 1] === '') {
                    ...
                    // 添加, 则设置当前岗位等级为目标岗位等级
                    $scope.model.gradeCursor = $scope.model.editNew.gradeList.length;
                    var node = {
                        name: '',
                        validName: 'grade' + $scope.model.gradeCursor,
                    ...
                    $scope.model.editNew.gradeList.push(node);

                    editNewJob.utils.getJobGradeValidResult();
                }
            },
            ...
            removeGrade: function (e, index) {
                e.preventDefault();
                $scope.model.editNew.gradeList.splice(index, 1);

                // 如果删除了当前正在编辑的岗位等级, 则默认显示第一个岗位
                if ($scope.model.gradeCursor === index) {
                    if ($scope.model.editNew.gradeList.length) {
                ...
                editNewJob.utils.getJobGradeValidResult();
                editNewJob.utils.refreshJobGradeLevel();
            },
            ...
            toggleGrade: function (e, index) {
                ...
                editNewJob.utils.reCalculateGradeLesson(index);
            },
            ...
            removeGradeLesson: function (e, index, lessonId) {
                ...
                var currentGrade = $scope.model.editNew.gradeList[$scope.model.gradeCursor];
                // 从当前岗位等级下的<课程集合>移除
                ...
                // 重新计算当前岗位等级的课程数、学分总数
                editNewJob.utils.reCalculateGradeLesson($scope.model.gradeCursor);
            },
            ...
            openLessonChoiceWindow: function (e) {
                ...
                if ($scope.model.gradeCursor !== -1) {
                    ...
                    // 加载能力项
                    //editNewJob.utils.loadAbility(
                    //    editNewJob.utils.initialLessonGrid
                    //);
                    editNewJob.utils.initialLessonTypeTree(true);
            ...
            toggleLessonDimension: function (e, dimension) {
                ...
                // 课程查询维度是<课程分类>
                dimension === 2 && editNewJob.utils.initialLessonTypeTree();
            ...
            choose: function (e, dataItem) {
                ...
                $scope.model.selectedLesson.push(editNewJob.utils.wrapGridRowData(dataItem));
            ...
            batchSelect: function (e) {
                ...
                for (i = 0; i < viewData.length; i++) {
                    ...
                    if (index === -1) {
                        ...
                        selectedLesson.push(editNewJob.utils.wrapGridRowData(dataItem));
                    }
                }
            },
            ...
            saveChoiceLesson: function (e) {
                ...
                $scope.model.editNew.gradeList[gradeCursor].lessonList = $scope.model.editNew.gradeList[gradeCursor].lessonList.concat($scope.model.selectedLesson);
                $scope.model.editNew.gradeList[gradeCursor].lessonIdList = $scope.model.editNew.gradeList[gradeCursor].lessonIdList.concat($scope.model.lessonGridParams.selectedLessonIdList);

                // 重新计算当前岗位等级的课程数、学分总数
                editNewJob.utils.reCalculateGradeLesson(gradeCursor);

                editNewJob.utils.resetLessonChoiceCache();
            ...
            create: function (e) {
                ...
                if ($scope.model.noSubmitIncrease && $scope.addJobForm.$valid && $scope.model.isJobGradePass) {
                    $scope.model.noSubmitIncrease = false;

                    jobService.save($scope.model.editNew).then(function (response) {
                        $scope.model.noSubmitIncrease = true;

                        if (response.status) {
                            $scope.globle.showTip('操作成功', 'success');
                            $scope.id = response.info;
            ...
            cancel: function (e) {
                e.preventDefault();
                $state.go('states.job', 'index').then(function () {
                    $state.reload();
                });
            }
        };
