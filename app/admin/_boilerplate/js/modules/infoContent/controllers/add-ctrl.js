define(function () {
    'use strict';
    return ['$scope', 'infoContentService','$state','$stateParams','KENDO_UI_EDITOR','$timeout', function ($scope, infoContentService,$state,$stateParams,KENDO_UI_EDITOR,$timeout) {
                        ...
                        infoContentService.getTopUnitInfoList().then(function(dataInfo){
                            ...
                            if(dataInfo.status){
                                infoContentService.getUserUnitId().then(function(dataTemp){
                                    $scope.model.newsDto.currentUnitId = dataTemp.info;
                                });
                            }else{
        ...
        $scope.events={
            ...
            selectParentCategory: function (e, dataItem) {
                ...
                if ($scope.model.newsDto.currentUnitId==null && $scope.model.isSuperAdmin==true){
                    $scope.globle.alert('提示', '请选择资讯发布单位');
                    return ;
                }
            ...
            publish:function(e,type){
                ...
                infoContentService.create($scope.model.newsDto).then(function(data){
                    if(data.status){
                        $scope.model.submitAble=false;
