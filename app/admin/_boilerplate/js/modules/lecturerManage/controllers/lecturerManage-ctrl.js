define(function () {
    'use strict';
    return ['$scope', 'lecturerManageService', 'KENDO_UI_GRID', 'kendo.grid', '$state', 'TabService', '$stateParams',
        function ($scope, lecturerManageService, KENDO_UI_GRID, kendoGrid, $state, TabService, $stateParams) {
            var utils;
            $scope.model = {
               dimension:1,
            ...
            $scope.events = {
                ...
                addLesson: function (e) {
                    ...
                    e.preventDefault();
                    $state.go('states.lecturerManage.add');
                },

                update: function (id) {
                    $state.go('states.lecturerManage.edit', {courseId: id});
                },

                deleteCourse: function (id) {
                    $scope.globle.confirm('提示', '是否需要删除课程', function (dialog) {
                        return lecturerManageService.deleteCourse(id).then(function (data) {
                            dialog.doRightClose();
                            if (data.status) {
...
/*
                 ...
                 *!/
                getAllRole: function () {
                    lecturerManageService.getAllRole().then(function (data) {
                ...
                enableAdministrator: function (e, dataItem) {
                    ...
                    $scope.globle.confirm('启用', '确定要启用该账号？', function (dialog) {
                        return lecturerManageService.enable(dataItem.userId).then(function (response) {
                            dialog.doRightClose();
                            if (response.status) {
                                dataItem.status = 1;
                ...
                resetPasswordExcute: function (e, adminAccountFormValid) {
                    if (adminAccountFormValid) {
                        lecturerManageService.resetPassword($scope.model.administratorId, $scope.model.resertPassword).then(function (data) {
                ...
                suspendAdministrator: function (e, dataItem) {
                    ...
                    $scope.globle.confirm('停用', '确定要停用该账号？', function (dialog) {
                        return lecturerManageService.suspend(dataItem.userId)
                ...
                updateAdministrator: function (e, dataItem) {
                    e.preventDefault();
                    $state.go('states.lecturerManage.edit', {
                ...
                view: function (e, dataItem) {
                    e.preventDefault();
                    $state.go('states.lecturerManage.view', {
                        administratorId: dataItem.userId
                    });
                },
                ...
                remove: function (e, dataItem) {
                    ...
                    $scope.globle.confirm('删除', '确定要删除该账号？', function (dialog) {
                        return lecturerManageService.remove(dataItem.userId).then(function (response) {
                            dialog.doRightClose();
                            if (response.status) {
            ...
            (function () {
                ...
                result.push('<button type="button" class="table-btn" has-permission="lecturerManage/detail" ng-click="events.view($event,dataItem)">详情</button>');
                result.push('<button type="button" class="table-btn" has-permission="lecturerManage/edit" ng-click="events.updateAdministrator($event,dataItem)">修改</button>');
                result.push('<button type="button" class="table-btn" has-permission="lecturerManage/resetPassword" ng-click="events.resetPassword(\'#: userId #\')">重置密码</button>');
                result.push('<button type="button"  class="table-btn" #: status==1?\'disabled\':\'\'#  has-permission="lecturerManage/enabled" ng-click="events.enableAdministrator($event,dataItem)">启用</button>');
                result.push('<button type="button"  class="table-btn" #: status==2?\'disabled\':\'\'#  has-permission="lecturerManage/disabled" ng-click="events.suspendAdministrator($event,dataItem)">禁用</button>');
            ...
            $scope.ui = {
                ...
                windows: {
                    addWindow: {//添加窗口
                        modal: true,
                        content: '@systemUrl@/views/lecturerManage/resetPassword.html',
                        visible: false,
                        title: false,
                        open: function () {
                            this.center();
                        ...
                        dataSource: {
                            transport: {
                                read: {
                                    ...
                                    url: '/web/admin/lecturerManage/findByQuery',
                                    data: function (e) {
