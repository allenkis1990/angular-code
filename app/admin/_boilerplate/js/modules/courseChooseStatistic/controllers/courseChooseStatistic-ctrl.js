define(function () {
    'use strict';
    return ['$scope', 'courseChooseStatisticService', 'HB_dialog', 'KENDO_UI_GRID', 'kendo.grid', '$state', 'TabService','$rootScope',
        function ($scope, courseChooseStatisticService, HB_dialog, KENDO_UI_GRID, kendoGrid, $state, TabService,$rootScope) {
            ...
            $scope.events = {
                ...
                exportCourseStatistics: function () {
                    ...
                    courseChooseStatisticService.exportCourseStatistics({
                        startTime: validateIsNull($scope.model.chooseParams.startCreateTime) === true ? null : $scope.model.chooseParams.startCreateTime,
                        endTime: validateIsNull($scope.model.chooseParams.endCrateTime) === true ? null : $scope.model.chooseParams.endCrateTime, /*+86399999*/
                ...
                openUnitTree: function () {
                    $scope.model.unitShow = !$scope.model.unitShow;
                },
                getUnitInfo: function (dataItem) {
                    courseChooseStatisticService.findUnitByParentId(dataItem.id).then(function (data) {
                        if (data.info.length==0) {
                            $scope.model.selectParam.unitId=dataItem.id;
                            $scope.model.unitName=dataItem.name;
                            $scope.model.unitShow = false;
                        }
                    });
                }
            };
            ...
            var dataSource = new kendo.data.HierarchicalDataSource({
                transport: {
                    read: function (options) {
                        ...
                        $.ajax({
                            url: '/web/admin/supplierResources/findUnitByParentId?parentId=' + id,
            ...
            $scope.ui = {
                ...
                courseGrid: {
                    options: {
                        ...
                        dataSource: {
                            transport: {
                                read: {
                                    ...
                                    url: '/web/admin/courseChooseStatistic/findStatisticData',
                                    data: function (e) {
                        ...
                        columns: [
                            {sortable: false, field: 'courseName', title: '课程名称', width: 130},
                            {sortable: false, field: 'createUnitName', title: '创建单位', width: 130},
                            {sortable: false, field: 'courseProviders', title: '课程提供商', width: 130},
                            {sortable: false, field: 'period', title: '课程学时', width: 90},
                            {sortable: false, field: 'chooseTimes', title: '累计选课次数', width: 50},
            ...
            function getSubjectAndYear () {
                ...
                //    }
                //});
                courseChooseStatisticService.findProvider().then(function (data) {
                    if (data.status) {
