    ...
    callback: function () {
        ...
        require.extUtil = {
            ...
            analysisUrl: function (str) {
                var reg = /^\/#/
                if (!/^\/(center|portal|admin)/.test(str) && !reg.test(str)) {
                    return str.replace(/^\/#/, '');
                }
                if (require.unitPath) {
                ...
                var solutions = angular.module('solutions', []),
                    getUserInfoPromise = $http.get('/web/login/login/getUserInfo.action', {
                        ...
                        .then(function (data) {
                            ...
                            solutions.constant('appStatus', data.data.status);
                            solutions.constant('appData', data);
                            ...
                            if (appConfig.userType !== 1 && appConfig.userType !== 0) {
                                $log.log('不是学员的用户不能进入学员中心');
