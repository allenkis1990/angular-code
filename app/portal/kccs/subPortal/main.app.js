 ...
 */
define([
    'kccs/subPortal/js/common/stateMapper',
    'kccs/subPortal/js/common/hbFrontCommon',
    'kccs/subPortal/js/common/hbSkuModule',
    'prometheus/modules/interceptor',
    'prometheus/modules/hb-artdialog',
    'kccs/subPortal/js/common/hb-notification',
    'kccs/subPortal/js/common/hbBootstrap',
    ...
    angular.module('app', [
            ...
            function ($stateProvider, $urlRouterProvider, $ocLazyLoadProvider,
                ...
                if (hb_domainConfig.units && hb_domainConfig.units.length) {
                    ...
                    $stateProvider.state('states', {
                        ...
                        abstract: true,
                        resolve: {
                            getTitle: ['$stateParams', '$timeout','$rootScope','$http', function ($stateParams, $timeout,$rootScope,$http) {
                                // 将当前的unitPath替换成 参数上面的
                                require.unitPath = $stateParams.unitPath;
                                //获取施教机构列表
                                $http.get('/web/portal/index/findTraningOrgnizations').success(function(data){
                                    if(data.status){
                                        $rootScope.orgnizationsList=data.info;
                                    }
                                });
        ...
        .run(['$rootScope', '$state', '$stateParams','hb.domainConfig','$timeout', function ($rootScope, $state, $stateParams,hb_domainConfig,$timeout) {
