define(function (mod) {
    ...
    return ['$scope', '$http', '$state', '$dialog', 'onTrainingService', 'homeService', '$stateParams', 'hbBasicData', '$rootScope', '$timeout', function ($scope, $http, $state, $dialog, onTrainingService, homeService, $stateParams, hbBasicData, $rootScope, $timeout) {
        ...
        $scope.model = {
            ...
            orgnization:'',
            showOrgnization:true,
            choseOrgnizationResult:null
        };
        ...
        $scope.$watch('model.clearEvent', function (nv) {
            if (nv) {
                ...
                $scope.model.orgnization='';
                $scope.model.showOrgnization=true;
                $scope.model.choseOrgnizationResult=null;
        ...
        $scope.events = {
            ...
            cacelOrgnizationSearch:function(){
                $scope.model.orgnization='';
                $scope.model.showOrgnization=true;
                $scope.model.choseOrgnizationResult=null;
                getCourseList();
            ...
            tabOrgnization:function(item){
                //点击item
                if(item){
                    $scope.model.orgnization=item.unitId;
                    $scope.model.showOrgnization=false;
                    $scope.model.choseOrgnizationResult={
                        name:'培训机构',
                        value:item.unitName
                    };
                }else{
        ...
        //获取施教机构
        $rootScope.$watch('orgnizationsList',function(nv){
            if(nv){
                $scope.model.orgnizationsList=nv;
                angular.forEach($scope.model.orgnizationsList,function(item){
                    if(item.unitName.length>5){
                        item.shortName=item.unitName.substr(0,5)+' ...';
                    }else{
                        item.shortName=item.unitName;
                    }
                });
            }
        });
        ...
        function getCourseList () {
            ...
            if ($scope.model.categoryType === 'TRAINING_CLASS_GOODS') {
                ...
                var params = {
                    ...
                    teachUnitId:$scope.model.orgnization
                };
                ...
                var params = {
                    ...
                    teachUnitId :$scope.model.orgnization, //施教机构id
                };
